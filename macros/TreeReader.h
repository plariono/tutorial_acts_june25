...
struct TrackStatesReader : public TreeReader
{
    ...
    TrackStatesReader(TTree *tree_, bool sortEvents) : TreeReader(tree_)
    {
        ...
        tree->SetBranchAddress("l_x_hit", &l_x_hit);
        tree->SetBranchAddress("l_y_hit", &l_y_hit);
    ...
    // The variables
    uint32_t eventId;
    std::vector<float> *l_x_hit =
        new std::vector<float>; ///<
    std::vector<float> *l_y_hit =
        new std::vector<float>; ///<
...
struct TrackSummaryReader : public TreeReader
{
    ...
    TrackSummaryReader(TTree *tree_, bool sortEvents) : TreeReader(tree_)
    {
        ...
        tree->SetBranchAddress("cov_eLOC0_eLOC1", &cov_eLOC0_eLOC1);
        tree->SetBranchAddress("cov_eLOC0_ePHI", &cov_eLOC0_ePHI);
        tree->SetBranchAddress("cov_eLOC0_eTHETA", &cov_eLOC0_eTHETA);
        tree->SetBranchAddress("cov_eLOC0_eQOP", &cov_eLOC0_eQOP);
        tree->SetBranchAddress("cov_eLOC1_ePHI", &cov_eLOC1_ePHI);
        tree->SetBranchAddress("cov_eLOC1_eTHETA", &cov_eLOC1_eTHETA);
        tree->SetBranchAddress("cov_eLOC1_eQOP", &cov_eLOC1_eQOP);
        tree->SetBranchAddress("cov_ePHI_eTHETA", &cov_ePHI_eTHETA);
        tree->SetBranchAddress("cov_ePHI_eQOP", &cov_ePHI_eQOP);
        tree->SetBranchAddress("cov_eTHETA_eQOP", &cov_eTHETA_eQOP);
    ...
    std::vector<float> *t_py = new std::vector<float>;
    std::vector<float> *t_pz = new std::vector<float>;
...
struct ParticleReader : public TreeReader
{
    ...
    // Get the number of particles per given event id
    UInt_t getNparticles(const uint32_t &eventNumber) const
    {
        // Find the start entry and the batch size for this event
        std::string eventNumberStr = std::to_string(eventNumber);
        std::string findStartEntry = "event_id<" + eventNumberStr;
        std::string findParticlesSize = "event_id==" + eventNumberStr;
        UInt_t startEntry = tree->GetEntries(findStartEntry.c_str());
        UInt_t nParticles = tree->GetEntries(findParticlesSize.c_str());
        if (nParticles == 0)
        {
            throw std::invalid_argument(
                "No particles found. Please check the input file.");
