...
def runALICE3(
    ...
    for ievt in range(events):
        ...
        context = AlgorithmContext(ialg, ievt, eventStore, 0)
...
if "__main__" == __name__:
    ...
    p.add_argument(
        "--geo-version", default="IRIS",help="Which geometry version to build. Choose between: withTOF, IRIS"
        )
    ...
    elif args.geo_version == "IRIS":

        detector = alice3_detector_woTOF_Iris.buildALICE3Geometry(
            geo_example_dir,
            material=not args.no_material,
        )

    else:
        raise RuntimeError(f"Geometry version {args.geo-version} not available. Choose between 'withTOF' or 'IRIS'")
